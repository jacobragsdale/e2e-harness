...
evaluated against, and `demo/pricing-app/e2e/` is a stamped copy with the suite the `e2e-setup` skill wrote.
...
- `./check.sh` is the gate: it stamps the template, runs `npm run check`, and runs the demo suite with the fresh
  harness. Run it before finishing a change.
...
- Change a skill only for a failure seen in an eval run: a headless `claude -p` session in a fresh stamped copy of
  the demo, run from its `e2e/`.
